...
async function buildContainerArgs(
  ...
  // Resource caps — opt-in. Some Docker Desktop configurations (esp.
  // on macOS) refuse --memory because the underlying Linux VM doesn't
  // expose enough RAM to honour the cap, exiting the container with
  // 127 before the entrypoint runs. Set NANOCLAW_CONTAINER_MEMORY=2g
  // on a multi-tenant Linux host where you actually need the cap.
  if (process.env.NANOCLAW_CONTAINER_MEMORY) {
    const memCap = process.env.NANOCLAW_CONTAINER_MEMORY;
